...
class CMUser extends CObject implements IHasSQL, ArrayAccess {
  ...
  public function Init() {
    try {
      ...
      $this->db->ExecuteQuery(self::SQL('insert into user'), array('doe', 'John/Jane Doe', 'yoooo@dbwebb.se', $password['algorithm'], $password['salt'], $password['password']));
  ...
  public function CreatePassword($plain, $algorithm=null) {
    $password = array(
      'algorithm'=>($algorithm ? $algorithm : CMauris::Instance()->config['hashing_algorithm']),
      'salt'=>null
    );
